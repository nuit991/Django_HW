...
class Momo_Consumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.channel_name = 'momo_channel'
    ...
    async def receive(self, text_data):  #text_data->前端丟的數據，這邊就是指product_name / max_pages
        ...
        print('MOMO_max_pages', max_pages)
    ...
    async def send_product_data(self, product_name, max_pages):
        print("MOMO_send_product_data called")

        #使用异步 for 循环从 scrape_momo_product 函数获取产品数据。
        async for prd_name, product_url, price, img_url in scrape_momo_product(product_name, max_pages): 
            data = {
                'prd_name': prd_name,
            ...
            print('Momo___Sending data:', data)
            print('\n')
